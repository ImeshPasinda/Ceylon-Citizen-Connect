...
function WaterUsersManage() {
  ...
  const [searchUserBywatermNo, setsearchUserBywatermNo] = useState("");
  ...
  const findUserByWatermNo = async () => {
    setLoading(true);

    // Simulate a delay of 1.5 seconds for the loading spinner
    setTimeout(async () => {
      try {
        const response = await axios.get(`/api/users/find-by-watermNo/${searchUserBywatermNo}`);
        if (response.data) {
        ...
        console.error("Error fetching user by watermNo:", error.message);
        setNotFound(true);
      } finally {
        setLoading(false);
      }
            ...
            <div class="modal-body" style={{ padding: '40px' }}>
              <div class="row">
                <div class="col-7 pr-1">
                  <input
                    type="text"
                    placeholder="Enter Water Account No..."
                    class="form-control"
                    value={searchUserBywatermNo}
                    onChange={(e) => setsearchUserBywatermNo(e.target.value)}
                  />
                </div>
                ...
                <div>
                  <div style={{ paddingTop: "25px" }}>
                    <div class="alert alert-success" role="alert">
                      Account No : {selectedUser.watermNo}
                    </div>
                  </div>
                  ...
                  <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                      <input type="text" class="form-control" id="accno" value={selectedUser.watermNo} disabled />
                    </div>
                  </div>
                  <div class="mb-3 row">
